...
import com.chatweb.user.exception.UserErrorCode;
import com.chatweb.user.dto.*;
import com.chatweb.user.entity.UserProfile;
import com.chatweb.user.repository.UserProfileRepository;
import com.chatweb.user.service.IUserProfileService;
...
public class UserProfileService implements IUserProfileService {
    ...
    @Value("${user.profile-cache-ttl-minutes:5}")
    ...
    private UserProfileResponse safeGetProfileFromCache(UUID accountId) {
        ...
        } catch (RedisConnectionFailureException | SerializationException ex) {
            log.warn(
    ...
    private void safePutProfileToCache(UUID accountId, UserProfileResponse response) {
        try {
            profileCache.put(accountId, response, Duration.ofMinutes(profileCacheTtlMinutes));
        } catch (RedisConnectionFailureException | SerializationException ex) {
            log.warn(
    ...
    public void updateProfile(UUID accountId, UpdateProfileRequest req) {
        ...
        evictProfileCacheAfterCommit(accountId);
    }
    ...
    public AvatarUploadResponse applyAvatarMetadata(UUID accountId, AvatarMetadataRequest request) {
        ...
        AvatarAssetMetadata metadata = toAvatarAssetMetadata(request);
    ...
    private AvatarAssetMetadata toAvatarAssetMetadata(AvatarMetadataRequest request) {
        return AvatarAssetMetadata.builder()
    ...
    private void validateAvatarMetadata(AvatarAssetMetadata request) {
        ...
        // CRITICAL: Validate that publicId matches user/avatar/ folder from upload-service policy
        if (!request.getPublicId().startsWith("user/avatar/")) {
            throw new BusinessException(CommonErrorCode.VALIDATION_ERROR, "Avatar publicId must come from user/avatar folder (upload-service confirm)");
        }
    }
    ...
    private void evictProfileCacheAfterCommit(UUID accountId) {
