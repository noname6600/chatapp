...
class PresenceServiceTest {
    ...
    private final Map<String, Long> connectionCounts = new HashMap<>();
    ...
    void setUp() {
        ...
        when(presenceEphemeralStatePort.incrementConnectionCount(any())).thenAnswer(invocation -> {
            UUID userId = invocation.getArgument(0);
            String key = userId.toString();
            long next = connectionCounts.getOrDefault(key, 0L) + 1L;
            connectionCounts.put(key, next);
            return next;
        });

        when(presenceEphemeralStatePort.decrementConnectionCount(any())).thenAnswer(invocation -> {
            UUID userId = invocation.getArgument(0);
            String key = userId.toString();
            long next = connectionCounts.getOrDefault(key, 0L) - 1L;
            if (next <= 0) {
        ...
        doAnswer(invocation -> {
            UUID userId = invocation.getArgument(0);
            connectionCounts.remove(userId.toString());
            return null;
        }).when(presenceEphemeralStatePort).clearConnectionCount(any());
    }
