...
class OAuthAuthServiceTest {
    ...
    private IIdentityProviderService identityProviderService;
        private AccountCreatedAfterCommitPublisher accountCreatedAfterCommitPublisher;
    ...
    void setUp() {
        ...
        accountCreatedAfterCommitPublisher = mock(AccountCreatedAfterCommitPublisher.class);
        oauthAuthService = new OAuthAuthService(accountRepository, identityProviderService, accountCreatedAfterCommitPublisher);
    }
    ...
    void loginGoogle_reusesExistingEmailAccountAndMarksItVerified() {
        ...
        verify(accountCreatedAfterCommitPublisher).publishAfterCommit(existing);
    }
    ...
    void loginGoogle_createsNewAccountWhenEmailDoesNotExist() {
        ...
        verify(accountRepository).save(any(Account.class));
                verify(accountCreatedAfterCommitPublisher).publishAfterCommit(created);
    }

    @Test
        void loginGoogle_returnsAccountId_whenNewAccountCreated() {
        UUID accountId = UUID.randomUUID();
        Account created = Account.builder()
                .id(accountId)
                .email("new@example.com")
    ...
    @Test
    void loginGoogle_returnsAccountId_whenExistingEmailReused() {
        UUID accountId = UUID.randomUUID();
        Account existing = Account.builder()
                .id(accountId)
                .email("user@example.com")
    ...
    void loginGoogle_doesNotPublish_whenReturningGoogleUser() {
        ...
        assertThat(actual).isEqualTo(accountId);
                verify(accountCreatedAfterCommitPublisher, never()).publishAfterCommit(any());
    }
}
