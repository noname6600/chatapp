...
import com.chatweb.gateway.controller.FallbackController;
...
class SecurityConfigIntegrationTest {
    ...
    @Test
    void protectedEndpoint_returns401_whenIssuerMismatch() {
        when(jwtDecoder.decode("issuer-mismatch-token"))
                .thenReturn(Mono.error(new BadJwtException("issuer mismatch")));

        webTestClient.get()
                .uri("/api/users/ping")
                .header(HttpHeaders.AUTHORIZATION, "Bearer issuer-mismatch-token")
                .exchange()
                .expectStatus()
                .isUnauthorized();
    }
