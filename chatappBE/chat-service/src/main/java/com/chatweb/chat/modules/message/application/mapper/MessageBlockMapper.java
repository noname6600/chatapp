package com.chatweb.chat.modules.message.application.mapper;

import com.chatweb.chat.modules.message.application.dto.request.AttachmentRequest;
import com.chatweb.chat.modules.message.application.dto.request.MessageBlockRequest;
import com.chatweb.chat.modules.message.application.dto.request.RoomInviteRequest;
import com.chatweb.chat.modules.message.application.dto.response.AttachmentResponse;
import com.chatweb.chat.modules.message.application.dto.response.MessageBlockResponse;
import com.chatweb.chat.modules.message.application.dto.response.RoomInviteResponse;
import com.chatweb.chat.modules.message.domain.enums.AttachmentType;
import com.chatweb.chat.modules.message.domain.enums.MessageBlockType;
import com.chatweb.chat.modules.message.domain.model.AttachmentDraft;
import com.chatweb.common.integration.chat.AttachmentPayload;
import com.chatweb.common.integration.chat.MessageBlockPayload;
import com.chatweb.common.integration.chat.RoomInvitePayload;
...
public class MessageBlockMapper {
    ...
    private MessageBlockRequest normalizeAssetBlock(MessageBlockRequest block) {
        ...
        // CRITICAL: Validate that attachment came from upload-service confirm flow
    ...
    private void validateAttachmentMetadata(AttachmentRequest attachment) {
        if (attachment.getPublicId() == null || attachment.getPublicId().isBlank()) {
            throw new BusinessException(
                    ChatErrorCode.ATTACHMENT_INVALID,
                    "Attachment publicId is required"
            );
        }

        // CRITICAL: Validate that publicId matches chat/attachments/ folder from upload-service policy
        if (!attachment.getPublicId().startsWith("chat/attachments/")) {
            throw new BusinessException(
                    ChatErrorCode.ATTACHMENT_INVALID,
                    "Attachment publicId must come from chat/attachments folder (upload-service confirm)"
            );
        }
        ...
        // Optional: validate URL is from Cloudinary
        if (!attachment.getUrl().startsWith("https://res.cloudinary.com/")) {
            throw new BusinessException(
                    ChatErrorCode.ATTACHMENT_INVALID,
                    "Attachment url must be from Cloudinary CDN"
            );
        }
    }
    ...
    private com.chatweb.common.integration.enums.AttachmentType toEventAttachmentType(AttachmentType type) {
        return switch (type) {
            case IMAGE -> com.chatweb.common.integration.enums.AttachmentType.IMAGE;
            case VIDEO -> com.chatweb.common.integration.enums.AttachmentType.VIDEO;
            case FILE -> com.chatweb.common.integration.enums.AttachmentType.FILE;
        };
    }
