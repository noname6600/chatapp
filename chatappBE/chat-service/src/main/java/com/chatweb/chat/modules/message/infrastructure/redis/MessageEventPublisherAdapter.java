...
public class MessageEventPublisherAdapter implements IMessageEventPublisher {
    ...
    @Override
    public void publishMessageCreated(ChatMessage message, List<ChatAttachment> attachments, List<UUID> mentionedUserIds) {
    ...
    private <T> void publishToKafka(String eventType, String key, T payload) {
        ...
        kafkaEventPublisher.publish(eventType, key, new EventEnvelope<>(
    ...
    private <T> void publishToKafka(String topic, String key, String eventType, T payload) {
        String eventId = UUID.randomUUID().toString();
        kafkaEventPublisher.publish(topic, key, new EventEnvelope<>(
                new EventMetadata(eventId, eventType, sourceService, Instant.now(), TraceContext.correlationIdOrEventId(eventId)),
                payload
        ));
    }
