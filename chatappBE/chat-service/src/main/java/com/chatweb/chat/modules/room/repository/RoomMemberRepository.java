...
public interface RoomMemberRepository extends JpaRepository<RoomMember, UUID> {
    ...
    @Query("select rm from RoomMember rm where rm.roomId = :roomId and rm.userId = :userId")
    Optional<RoomMember> findByRoomIdAndUserIdForUpdate(UUID roomId, UUID userId);

    List<RoomMember> findByRoomId(UUID roomId);

    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("select rm from RoomMember rm where rm.roomId = :roomId")
    List<RoomMember> findByRoomIdForUpdate(UUID roomId);
