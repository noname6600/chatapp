...
class MessageQueryServiceTest {
    ...
    void getLatestMessages_passesCurrentUserIdToReactionSummaryQuery() {
        ...
        verify(roomMembershipGuard).ensureRoomMember(roomId, currentUserId);
    ...
    void getMessagesBefore_passesCurrentUserIdToReactionSummaryQuery() {
        ...
        @Test
        void getLatestMessages_throwsForbidden_whenUserNotRoomMember() {
                UUID currentUserId = UUID.randomUUID();
                UUID roomId = UUID.randomUUID();

                doThrow(new BusinessException(CommonErrorCode.FORBIDDEN, "Not a room member"))
                                .when(roomMembershipGuard)
                                .ensureRoomMember(roomId, currentUserId);

                assertThatThrownBy(() -> service.getLatestMessages(currentUserId, roomId, 50))
