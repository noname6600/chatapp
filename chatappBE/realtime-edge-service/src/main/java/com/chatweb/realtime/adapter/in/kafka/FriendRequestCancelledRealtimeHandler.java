...
public class FriendRequestCancelledRealtimeHandler implements KafkaEventHandler<Object> {

    private final FriendshipRealtimeDeliveryService friendshipDeliveryService;
    private final ObjectMapper objectMapper;

    @Override
    public String eventType() { return "friend.request.cancelled"; }
    ...
    public void handle(EventEnvelope<Object> event) {
        ...
        wsPayload.put("type", "friend.request.cancelled".substring("friend.request.".length()).toUpperCase());

        if (senderId != null) friendshipDeliveryService.deliverToUser(senderId, "friendship.request.cancelled", eventId, wsPayload);
        if (recipientId != null) friendshipDeliveryService.deliverToUser(recipientId, "friendship.request.cancelled", eventId, wsPayload);
    }
