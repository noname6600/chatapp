...
public class FriendRequestAcceptedRealtimeHandler implements KafkaEventHandler<Object> {

    private final FriendshipRealtimeDeliveryService friendshipDeliveryService;
    private final ObjectMapper objectMapper;

    @Override
    public String eventType() { return "friend.request.accepted"; }
    ...
    public void handle(EventEnvelope<Object> event) {
        ...
        wsPayload.put("type", "friend.request.accepted".substring("friend.request.".length()).toUpperCase());

        if (senderId != null) friendshipDeliveryService.deliverToUser(senderId, "friendship.request.accepted", eventId, wsPayload);
        if (recipientId != null) friendshipDeliveryService.deliverToUser(recipientId, "friendship.request.accepted", eventId, wsPayload);
    }
