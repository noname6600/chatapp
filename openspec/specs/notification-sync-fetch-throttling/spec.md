# notification-sync-fetch-throttling Specification

## Purpose
Define single-flight and rate-limited notification sync fetch behavior for reconnect-driven convergence paths while preserving responsiveness for explicit user actions.
...
- **WHEN** multiple reconnect or open lifecycle events request notification sync while a sync request is already in flight
...
- **WHEN** websocket reconnect or open events occur repeatedly within the configured minimum sync interval
