## Purpose
Guard the password change endpoint against null authentication principals, ensuring that requests without a resolved `JwtPrincipal` produce a deterministic auth failure response rather than a server-side `NullPointerException`, while preserving the existing successful behavior for authenticated requests.
