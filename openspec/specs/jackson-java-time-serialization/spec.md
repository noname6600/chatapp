...
All Spring `@Bean ObjectMapper` instances in the application context must include `JavaTimeModule` (or equivalent time-type support) so that `java.time.Instant` values can be serialized to and deserialized from JSON.
