...
The deployment SHALL route production traffic through Nginx based on request host so frontend and API traffic are separated by domain.

#### Scenario: Frontend domain request
- **WHEN** a client requests the configured frontend host
- **THEN** Nginx forwards the request to the frontend runtime target

#### Scenario: API domain request
- **WHEN** a client requests the configured API host
...
- **WHEN** a client initiates a WebSocket upgrade request through the API domain
