...
The system SHALL display a sticky banner above the message list showing the number of unread messages in the current room when unread state indicates unread messages, and SHALL keep the count consistent with last-read sequence and authoritative latest sequence. Temporary optimistic placeholders MAY appear in the rendered list, but they MUST NOT be treated as authoritative latest-sequence state for unread or behind-latest calculations.
...
#### Scenario: Pending image send does not create overflow-like behind-latest count
- **WHEN** the sender uploads or sends a single image message and a pending optimistic placeholder exists before confirmation
- **THEN** the top indicator MUST NOT display an overflow-like value such as `9007199254740982+ messages behind latest`
- **AND** the displayed count remains bounded to valid confirmed-message state
