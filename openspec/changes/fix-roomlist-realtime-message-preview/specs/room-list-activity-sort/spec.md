...
The client SHALL display rooms sorted by the timestamp of the most recently sent message (or room creation time if no messages yet), with the most recently active room at the top, and SHALL update each affected room's last-message preview fields in real time when websocket message events arrive.
...
#### Scenario: Background room preview updates without page refresh
- **WHEN** a `ws_message_sent_event` arrives for a background room
- **THEN** that room's preview fields (last sender, preview text, and latest timestamp) update in room list state immediately
- **AND** the updated preview is visible in the room list without manual browser refresh
